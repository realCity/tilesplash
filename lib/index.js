...
function stringifyProtobuf(layers, tile) {
  var tileData = {},
      name, tileindex, vtTile;
  for(name in layers) if(layers.hasOwnProperty(name)) {
    tileindex = geojsonVt(layers[name]);
    vtTile = tileindex.getTile(tile.z, tile.x, tile.y);
    if (vtTile == null) {
      vtTile = {features: []};
    }
    tileData[name] = vtTile;
  }
  return vtpbf.fromGeojsonVt(tileData);
}
